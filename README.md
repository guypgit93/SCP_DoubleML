...
| `08_stage5_stacked_ml_did.R` | Stage 5: jointly estimated, vector-outcome DML DiD across all ten items (shared complete-case sample, multivariate outcome-regression learner). | `stage5_stacked_item_did.csv` |
| `09_placebo_wales_ni.R` | Falsification check: re-estimates the headline spec with Wales, and separately Northern Ireland, as the pseudo-treated unit against England, at SCP's real rollout date. | `placebo_wales_ni.csv/.tex` |
| `10_make_latex_tables.R` | Reformats the CSV outputs of scripts 03–09 into the final `.tex` tables `\input` directly into the dissertation (`Sections/Tables/`). Run last. | All `Sections/Tables/*.tex` files |
